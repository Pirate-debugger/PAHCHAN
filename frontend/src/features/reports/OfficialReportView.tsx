...
export const OfficialReportView: React.FC<OfficialReportViewProps> = ({
  ...
  const hash = reportData.security_seal?.audit_hash || 'SHA256:7F89B24C09A888124C49D2678FE19E0E782';

  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      
      {/* Top Controls Bar (Hidden during print) */}
      <div className="no-print bg-white rounded-2xl border border-slate-200/90 p-4 shadow-subtle flex items-center justify-between">
        <Button
          variant="secondary"
          size="sm"
          onClick={onBack}
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-slate-500 font-medium hidden sm:inline">
            Border Checkpoint Forensic Dossier Ready for Print
          </span>
          <Button
            variant="sovereign"
            size="sm"
          ...
          >
            Print Official Dossier (A4)
          </Button>
        </div>
      </div>

      {/* Printable Official Document Sheet */}
      <div className="bg-white rounded-2xl border border-slate-300 p-8 sm:p-10 shadow-subtle space-y-6 print:border-none print:p-2 print:shadow-none font-sans">
        ...
        <div className="border-b-2 border-slate-900 pb-5 text-center space-y-1.5 relative">
          <div className="flex items-center justify-center gap-2 mb-1">
            ...
            <span className="font-bold tracking-widest text-xs uppercase text-slate-900 font-mono">
          ...
          <p className="text-[10px] font-mono text-slate-500 pt-0.5 uppercase tracking-wide">
            OFFICIAL BORDER CHECKPOINT EXAMINATION &amp; FORENSIC AUDIT DOSSIER &bull; BORDER SECURITY DIVISION
          </p>
        </div>

        {/* Case Metadata Grid & Verification QR Seal */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-4 rounded-xl bg-slate-50 border border-slate-200">
          <div className="md:col-span-9 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            ...
            <div>
              ...
              <span className="text-slate-800 font-semibold">{new Date(reportData.screening_date).toLocaleString()}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="font-bold text-slate-900">Checkpoint ICP-04</span>
            </div>
          </div>

          {/* Cryptographic QR Seal Box */}
          <div className="md:col-span-3 flex flex-col items-center justify-center p-2 rounded-lg bg-white border border-slate-200 text-center">
            ...
            <span className="text-[8px] font-mono text-slate-600 mt-1 uppercase tracking-wider font-bold">
              Cryptographic Seal
            </span>
          </div>
        </div>
        ...
        <div className="border border-slate-200 rounded-xl p-4.5 space-y-3">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
            <span className="text-xs font-bold uppercase text-slate-800 tracking-wider">
              1. Automated Screening Risk Assessment
            </span>
          ...
          {risk && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div>
                ...
                <span className="font-bold text-slate-900 text-sm mt-0.5 block">{risk.primary_concern}</span>
              </div>
              <div>
                <span className="text-slate-500 block">System Recommendation:</span>
                <span className="font-bold text-slate-800 mt-0.5 block">{risk.recommendation}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="border border-slate-200 rounded-xl p-4.5 space-y-3">
          <span className="text-xs font-bold uppercase text-slate-800 block border-b border-slate-100 pb-2.5 tracking-wider">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
            {Object.entries(reportData.extracted_identity || {}).map(([k, val]: [string, any]) => {
              ...
              return (
                <div key={k} className="p-2.5 rounded-lg bg-slate-50/70 border border-slate-200">
                  <span className="text-[10px] text-slate-400 uppercase font-semibold block">{val.label}</span>
                  <span className="font-mono font-bold text-slate-900 text-sm">{val.value || 'N/A'}</span>
                  {val.is_edited && (
                    <span className="text-[9px] text-amber-800 font-semibold block mt-0.5">[Audited Correction]</span>
                  )}
                </div>
              );
            })}
          ...
          {reportData.forensic_findings && reportData.forensic_findings.length > 0 ? (
            <div className="space-y-2.5">
              {reportData.forensic_findings.map((f: any) => (
                <div key={f.id} className="p-3.5 rounded-lg bg-slate-50 border border-slate-200 text-xs flex justify-between items-start gap-4">
                  <div>
                    <span className="font-bold text-slate-900 text-xs">{f.title}</span>
                  ...
                  <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-rose-100 text-rose-800 border border-rose-300 shrink-0">
            ...
            <p className="text-xs text-slate-500">
              No forensic tampering indicators identified. Document structure matches authentic baseline template.
            </p>
          )}
        </div>
        ...
        {reportData.face_verification && (
          <div className="border border-slate-200 rounded-xl p-4.5 space-y-2 text-xs">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
              <span className="font-bold uppercase text-slate-800 tracking-wider">4. Biometric Face Verification</span>
              <span className={`font-bold font-mono px-2.5 py-0.5 rounded-md border text-[11px] ${
        ...
        <div className="border border-slate-200 rounded-xl p-4.5 space-y-4">
          <span className="text-xs font-bold uppercase text-slate-800 block border-b border-slate-100 pb-2.5 tracking-wider">
            5. Frontline Officer Determination &amp; Legal Sign-Off
          </span>
          {decision ? (
            <div className="grid grid-cols-2 gap-4 text-xs">
              <div>
                ...
                {decision.notes && (
                  <p className="text-slate-600 mt-1 italic bg-slate-50 p-2 rounded border border-slate-200">
          ...
          <div className="pt-8 grid grid-cols-2 gap-8 text-xs text-slate-600">
            <div className="border-t border-slate-400 pt-1.5 text-center font-medium">
        ...
        <div className="p-4 rounded-xl bg-slate-100 border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-[10px] font-mono text-slate-500">
          <div>
            <span className="font-bold text-slate-800 block">CHAIN OF CUSTODY SECURITY HASH</span>
          ...
          <div className="text-right sm:text-right">
            ...
            <span>BORDER CHECKPOINT UNIT &bull; Sashastra Seema Bal</span>
          </div>
        </div>
